...
function SubmitData() {
    ...
    var datavar = {
        ...
        'RegistrationNo': $('#ddlRegistration').val(),
    ...
    $.when(
        $.ajax({
            type: "POST",
            contentType: "application/json; charset=utf-8",
            url: '/WebApp/Kiosk/DTEV2/Migration3Verify.aspx/InsertNewMigration',
...
function BindRollNoDetail() {
    ...
    if (RollNoDetail == null || RollNoDetail == " " || RollNoDetail == "0") {
        ...
        $.ajax({
            type: "POST",
            contentType: "application/json; charset=utf-8",
            url: "/WebApp/Kiosk/DTEV2/Migration3Verify.aspx/GetRollNoDetails",
...
function GetMigrationData() {
    ...
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: '/WebApp/Kiosk/DTEV2/Migration3verify.aspx/GetMigrationData',
        data: '{"regno":"' + ddlregNo + '"}',
        processData: false,
        dataType: "json",
        success: function (response) {
            ...
            if (obj[0]['BranchName'] != null && obj[0]['BranchName'] != "") {
                $("#ddlBranch").val("");
                $("#ddlBranch").val(obj[0]['BranchName']);
                $("#ddlBranch").prop("disabled", true);
            }

            if (obj[0]['instituteName'] != null && obj[0]['instituteName'] != "") {
                $("#ddlInstitute").val("");
                $("#ddlInstitute").val(obj[0]['instituteName']);
                $("#ddlInstitute").prop("disabled", true);
            }

            if (obj[0]['InstituteLeavingDate'] != null && obj[0]['InstituteLeavingDate'] != "") {
                $("#txtDate").val("");
                var LivDate = obj[0]['InstituteLeavingDate'];
                var D1 = LivDate.split('/');
                var lday = D1[0];
                var lmon = D1[1];
                var lyear = D1[2];
                LivDate = lday + "-" + lmon + "-" + lyear;
                $("#txtDate").val(LivDate);
                $("#txtDate").prop("disabled", true);
           }

            if (obj[0]['ExaminationDetails'] != null && obj[0]['ExaminationDetails'] != "") {
                $("#ddlReason").val("");
                $("#ddlReason").val(obj[0]['ExaminationDetails']);
                $("#ddlReason").prop("disabled", true);
            }

            if (obj[0]['CertificateReason'] != null && obj[0]['CertificateReason'] != "") {
                $("#txReason").val("");
                $("#txReason").val(obj[0]['CertificateReason']);
                $("#txReason").prop("disabled", true);
            }

            //$("#SelectRollNo").val(obj[0]['RollNoValue']);
            //$('#SelectRollNo').prop("disabled", true);
        },
        error: function (a, b, c) {
            alert("Unable to fetch Requested Data : " + a.responseText);
        }
    });
...
function ValidateForm() {
    ...
    if (txtRegistrationNo == null || txtRegistrationNo == "") {
        ...
        $('#ddlRegistration').attr('style', 'border:1px solid #d03100 !important;');
        $('#ddlRegistration').css({ "background-color": "#fff2ee" });
        opt = 1;
    } else if (txtRegistrationNo != null) {
        $('#ddlRegistration').attr('style', 'border:1px solid #cdcdcd !important;');
        $('#ddlRegistration').css({ "background-color": "#ffffff" });
    }
