<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ResponseICICI.aspx.cs" Inherits="CitizenPortal.PG.ResponseICICI" %>
...
Key: 0: value = 0 (=> 13. TPS)
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        //  string reference2 = "";
        // string key = "";
        Bank_ResponseString = HttpContext.Current.Request.Form.ToString();
        InsertResponse(Bank_ResponseString, Createdby, CreatedOn);//
        if (!string.IsNullOrEmpty(Request.Form["Response Code"]))
        {
            Bank_ResponseCode = Request.Form["Response Code"];
        }
        if (!string.IsNullOrEmpty(Request.Form["Unique Ref Number"]))
        {
            Bank_UniqueRefNumber = Request.Form["Unique Ref Number"];
        }
        if (!string.IsNullOrEmpty(Request.Form["Service Tax Amount"]))
        {
            Bank_ServiceTax = Request.Form["Service Tax Amount"];
        }
        if (!string.IsNullOrEmpty(Request.Form["Processing Fee Amount"]))
        {
            Bank_ProcessFee = Request.Form["Processing Fee Amount"];

        }
        if (!string.IsNullOrEmpty(Request.Form["Total Amount"]))
        {
            Bank_TotalAmt = Request.Form["Total Amount"];
        }
        if (!string.IsNullOrEmpty(Request.Form["Transaction Amount"]))
        {
            Bank_TransactionAmt = Request.Form["Transaction Amount"];
        }

        if (!string.IsNullOrEmpty(Request.Form["Transaction Date"]))
        {
            Bank_TransDate = Request.Form["Transaction Date"];
        }
        if (!string.IsNullOrEmpty(Request.Form["Interchange Value"]))
        {
            Bank_Interchangevalue = Request.Form["Interchange Value"];
        }
        if (!string.IsNullOrEmpty(Request.Form["TDR"]))
        {
            Bank_TDR = Request.Form["TDR"];
        }
        if (!string.IsNullOrEmpty(Request.Form["Payment Mode"]))
        {
            Bank_PaymentMode = Request.Form["Payment Mode"];
        }
        if (!string.IsNullOrEmpty(Request.Form["SubMerchantId"]))
        {
            Bank_SubMerchantId = Request.Form["SubMerchantId"];
        }
        if (!string.IsNullOrEmpty(Request.Form["ReferenceNo"]))
        {
            BANK_ReferenceNo = Request.Form["ReferenceNo"];
            ReferenceNo = Request.Form["ReferenceNo"];
        }
        if (!string.IsNullOrEmpty(Request.Form["ID"]))
        {
            Bank_MerchantId = Request.Form["ID"];
        }

        if (!string.IsNullOrEmpty(Request.Form["RS"]))
        {
            Bank_RS = Request.Form["RS"];
        }
        if (!string.IsNullOrEmpty(Request.Form["TPS"]))
        {
            Bank_TPS = Request.Form["TPS"];
        }
        ...
        //         key = HttpContext.Current.Request.Form[i];
        //        if (key.StartsWith("PG") == true && reference2=="")
        //        {
        //            reference2 = key;
        //        }
        //        Bank_ResponseString += "~Key: " + HttpContext.Current.Request.Form[i] + ": value = " + key;
        //    }
        ...
        //    Bank_ResponseString += "~Key: " + HttpContext.Current.Request.Form[key] + ": value = " + value;
        ...
        // reference2  = HttpContext.Current.Request.Form.AllKeys.Where(k => k.StartsWith("PG")).ToString();



        //  Bank_ResponseCode = HttpContext.Current.Request.Form[0];
        //if (Bank_ResponseCode == "E000")
        //{
        //    Bank_UniqueRefNumber = HttpContext.Current.Request.Form[1];
        //    Bank_ServiceTax = HttpContext.Current.Request.Form[2];
        //    Bank_ProcessFee = HttpContext.Current.Request.Form[3];
        //    Bank_TotalAmt = HttpContext.Current.Request.Form[4];
        //    Bank_TransactionAmt = HttpContext.Current.Request.Form[5];
        //    Bank_TransDate = HttpContext.Current.Request.Form[6];
        //    Bank_Interchangevalue = HttpContext.Current.Request.Form[7];
        //    Bank_TDR = HttpContext.Current.Request.Form[8];
        //    Bank_PaymentMode = HttpContext.Current.Request.Form[9];
        //    Bank_SubMerchantId = HttpContext.Current.Request.Form[10];
        //    BANK_ReferenceNo = HttpContext.Current.Request.Form[11];
        //    Bank_MerchantId = HttpContext.Current.Request.Form[12];
        //    Bank_RS = HttpContext.Current.Request.Form[13];
        //    Bank_TPS = HttpContext.Current.Request.Form[14];
        //    ReferenceNo = HttpContext.Current.Request.Form[11];
        //}
        //else
        //{
        //    Bank_UniqueRefNumber = HttpContext.Current.Request.Form[1];
        //    Bank_ServiceTax = HttpContext.Current.Request.Form[2];
        //    Bank_ProcessFee = HttpContext.Current.Request.Form[3];
        //    Bank_SubMerchantId = HttpContext.Current.Request.Form[4];
        //    BANK_ReferenceNo = HttpContext.Current.Request.Form[5];
        //    ReferenceNo = HttpContext.Current.Request.Form[5];
        //    Bank_MerchantId = HttpContext.Current.Request.Form[6];
        //    Bank_Interchangevalue = HttpContext.Current.Request.Form[7];
        //    Bank_TDR = HttpContext.Current.Request.Form[8];

        //}

        //  ReferenceNo = ReferenceNo.StartsWith("PG") == true ? ReferenceNo : reference2;

        DataSet ds1 = new DataSet();
        ds1 = GetserviceList(ReferenceNo);
        if (ds1 == null)
        {
            Response.Write("Invalid Record");
            return;
        }
        else
        {
            if (ds1.Tables[0].Rows.Count > 0)
            {
                ...
                returnURl = ds1.Tables[0].Rows[0]["PGRetrunURL"].ToString();
            }
            else
            {
        ...
        SVCID = HttpUtility.UrlEncode(KeyClass.Encrypt(ServiceID));
        REFNO = HttpUtility.UrlEncode(KeyClass.Encrypt(ReferenceNo));
        PG_Status = HttpUtility.UrlEncode(KeyClass.Encrypt(Bank_ResponseCode));
        string appid = HttpUtility.UrlEncode(KeyClass.Encrypt(AppID));
        //   string transid = HttpUtility.UrlEncode(Encrypt(Bank_UniqueRefNumber));
        if (Bank_TransDate == "")
        {
            Bank_TransDate = "";
        }
        ...
        string date = HttpUtility.UrlEncode(KeyClass.Encrypt(Bank_TransDate));
        string transAmt = HttpUtility.UrlEncode(KeyClass.Encrypt(Bank_TotalAmt));
        string pm = HttpUtility.UrlEncode(KeyClass.Encrypt(Bank_PaymentMode));
        string pname = HttpUtility.UrlEncode(KeyClass.Encrypt(pgname));

        if (ServiceID == "852")
        {
            
            t_URL += ConfigurationManager.AppSettings["SuccessUrl"].ToString(); //"=" + ServiceID + "&AppID=" + ReferenceNo + "&Status=Y";
            t_URL += "?SvcID=" + SVCID + "&AppID=" + appid + "&TransID=" + REFNO + "&Transdate=" + date;
            t_URL += "&TransAmt=" + transAmt + "&PaymentMode=" + pm + "&PGName=" + pgname + "&Status=" + PG_Status;
        ...
        {
            t_URL = ConfigurationManager.AppSettings["SuccessUrl"].ToString();//"=" + ServiceID + "&AppID=" + ReferenceNo + "&Status=Y";
            string url = t_URL + "?SvcID=" + SVCID + "&TransID=" + REFNO + "&Status=" + PG_Status;
    ...
    protected bool InsertResponse(string Response, string CreatedBy, string CreatedOn)
    {
    ...
    protected DataSet GetserviceList(string Pgsequence)
    {
        ...
        SqlConnection con = new SqlConnection(connStr);
        SqlCommand cmd = new SqlCommand("Select  ServiceID ,AppID ,PGRetrunURL from PGRequestTB where PGSequenceNo =@PGSequenceNo ", con);
